/** @file 自定义标题栏 v12.1 - 毛玻璃质感+丝滑动画+个人信息弹窗+切换用户+注销授权 */
...
export default function TitleBar() {
  ...
  // #21 修复：移除双击标题栏关闭标签页的非标准交互
  // 标签页关闭统一使用关闭按钮(x)和右键菜单
  ...
  const userMenuItems: MenuProps['items'] = [
    ...
    { key: 'revoke-license', label: '注销授权', icon: <DisconnectOutlined />, danger: true },
  ]
  ...
  const handleUserMenuClick: MenuProps['onClick'] = ({ key }) => {
    switch (key) {
      ...
      case 'revoke-license':
        Modal.confirm({
          title: '注销授权',
          content: '注销后将清除本地激活状态，应用需要重新输入授权码才能使用。确定要注销授权吗？',
          okText: '确认注销',
          cancelText: '取消',
          okButtonProps: { danger: true },
          onOk: async () => {
            try {
              const { revoke } = useLicenseStore.getState()
              await revoke()
              logout()
              resetTabs()
              message.success('授权已注销，请重新激活')
              navigate('/activation')
            } catch (err: unknown) {
