...
4.5. **【新增】版本控制检查点（强制）**：
    > 参考 `version-release` 技能的检查点 2
    
    如果功能涉及数据库变更，技术方案中**必须**包含"数据库迁移方案"章节：
    ...
    如果不涉及数据库变更，标注：✅ 无数据库变更
5.  **【新增】UI设计规范引用**：
    *   检查 `specs/ui-design/` 目录是否存在
    *   如果存在，读取 `specs/ui-design/components.md`
    *   在技术方案中引用相关组件规范
    *   确保页面设计遵循UI规范
6.  **技术决策说明**：
...
9.  **文档生成**：
    *   读取 `templates/feature/tech-design-template.md`。
    *   填充内容，生成 Markdown 文档。
10. **最终交付**：当文档内容被用户确认后，请将其保存到 `specs/features/{功能名称}_技术方案.md`（与需求文档在同一目录下）。
...
### 如果UI规范不存在
提示用户：
> "检测到项目尚未生成UI设计规范，建议先执行 `ui-design-system` 技能生成设计规范，或继续使用默认样式。"

## 输出模板

1. 读取 `templates/feature/tech-design-template.md`。
2. 填入设计好的内容。
3. 保存为 `specs/features/{功能名称}_技术方案.md`。

## 交互准则
*   **严谨性优先**：技术方案必须准确、可执行，不能有模糊描述。
*   **引导式设计**：如果用户对技术细节不确定，**必须提供选项**，且**必须根据项目现状给出推荐**。
    - *Bad*: "你想用什么缓存方案？"
...
*   **【新增】UI规范遵循**：前端设计必须引用项目UI规范，确保一致性。
