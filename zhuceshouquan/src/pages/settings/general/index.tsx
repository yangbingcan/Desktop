...
import { getMachineId, fetchLicenseStatus, type LicenseStatusResult } from '../../../services/licenseService'
    ...
    <div className="space-y-4">
      <Tabs
        ...
        items={[
          ...
          { key: 'license', label: '授权信息', icon: <SafetyCertificateOutlined /> },
...
function LicenseInfoTab() {
  ...
  const handleCopyMachineId = () => {
    const mid = licenseStatus?.machine_id || machineId
    if (!mid) return
  ...
  const handleCopyLicenseCode = () => {
    const code = licenseStatus?.license_code
    if (!code) return
  ...
  /** 授权码星号隐藏：保留前8后8，中间用 **** 替换 */
  const maskLicenseCode = (code: string) => {
    if (code.length <= 20) return code
    return code.slice(0, 8) + '****' + code.slice(-8)
  }

  const isExpired = licenseStatus?.expiry
    ? dayjs().format('YYYY-MM-DD') > licenseStatus.expiry
    : false
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl p-6" style={{ background: 'var(--gl-card-bg)', border: '1px solid var(--gl-border)' }}>
          <div className="flex items-center gap-2 mb-4">
            <SafetyCertificateOutlined style={{ color: 'var(--gl-primary)', fontSize: 18 }} />
            <span className="text-[15px] font-semibold" style={{ color: 'var(--gl-text-primary)' }}>
              授权状态
            </span>
          </div>
          <Descriptions column={1} size="small" colon={false} labelStyle={{ color: 'var(--gl-text-secondary)', fontSize: 13, width: 100 }} contentStyle={{ color: 'var(--gl-text-primary)', fontSize: 13 }}>
                ...
                <Descriptions.Item label="有效期">
                  {licenseStatus.expiry ? (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                      <span>{licenseStatus.expiry}</span>
                      {isExpired ? (
                    ...
                    <Tag color="success">永久授权</Tag>
                  )}
                </Descriptions.Item>
                {licenseStatus.app_version && (
                  <Descriptions.Item label="激活版本">
                    v{licenseStatus.app_version}
                  </Descriptions.Item>
                )}
              </>
            )}
            <Descriptions.Item label="当前机器码">
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontFamily: 'monospace', fontSize: 13 }}>
                  {licenseStatus?.machine_id || machineId || '-'}
                </span>
                {(licenseStatus?.machine_id || machineId) && (
                  <Tooltip title="复制机器码">
                    <Button
                      type="text"
                      size="small"
                      icon={copiedMid ? <CheckOutlined /> : <CopyOutlined />}
                      onClick={handleCopyMachineId}
                      style={{ color: 'var(--gl-primary)', padding: '0 4px' }}
                    />
                  </Tooltip>
                )}
              </span>
              ...
              <Descriptions.Item label="授权码">
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontFamily: 'monospace', fontSize: 13, wordBreak: 'break-all' }}>
                    {maskLicenseCode(licenseStatus.license_code)}
                  </span>
                  <Tooltip title="复制授权码">
                    <Button
                      type="text"
        ...
        {/* 说明 */}
        <div className="rounded-xl p-5" style={{ background: 'var(--gl-primary-bg)', border: '1px solid var(--gl-border)' }}>
          <div className="text-[12px] leading-relaxed" style={{ color: 'var(--gl-text-secondary)' }}>
            <p className="mb-1"><strong>说明：</strong></p>
            <p>• 授权码绑定当前机器，不可跨机器使用</p>
            <p>• 如需更换机器或续期，请复制上方机器码联系技术服务人员</p>
            <p>• 授权过期后系统将无法使用，请及时续期</p>
          </div>
        </div>
      </div>
    </Spin>
