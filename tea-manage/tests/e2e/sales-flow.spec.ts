...
test.describe('零售收银流程', () => {
    ...
    test('页面应正确加载并显示商品', async ({ page }) => {
        ...
        await expect(page.locator('.product-card').first()).toContainText('西湖龙井')
        await expect(page.locator('.product-card').nth(1)).toContainText('碧螺春')
    })
    ...
    test('分类筛选应能过滤商品', async ({ page }) => {
        // 点击"绿茶"分类标签
        await page.locator('.category-tab', { hasText: '绿茶' }).click()
    ...
    test('点击商品应添加到购物车', async ({ page }) => {
        ...
        // 验证购物车区域有内容（购物清单不为空）
        const cartArea = page.locator('.cart-area')
        await expect(cartArea).toBeVisible()

        // 验证购物车不再显示"购物车为空"
        await expect(page.locator('text=购物车为空')).not.toBeVisible()
    })
    ...
    test('完整结算流程：添加商品 → 结算成功（验证 BUG 修复）', async ({ page }) => {
        ...
        // 如果出现单位选择弹窗，选择第一个单位
        const unitModal = page.locator('.n-modal').filter({ hasText: '选择单位' })
        if (await unitModal.isVisible({ timeout: 1000 }).catch(() => false)) {
    ...
    test('会员识别功能应能识别手机号', async ({ page }) => {
        // 注入带会员数据的 mock
        await injectMockTauri(page, {
            get_member_by_phone: () => mockData.members.list[0]
        })
        ...
        if (await phoneInput.isVisible({ timeout: 2000 }).catch(() => false)) {
            ...
            await page.locator('button', { hasText: '识别' }).click()

            // 等待会员信息显示
            await page.waitForTimeout(500)

            // 验证会员信息显示
            await expect(page.locator('.member-info')).toBeVisible({ timeout: 2000 })
            await expect(page.locator('.member-name')).toContainText('张三')
        }
    })
    ...
    test('清空购物车按钮应能清空购物车', async ({ page }) => {
        ...
        // 验证购物车恢复空状态
        await expect(page.locator('text=购物车为空')).toBeVisible({ timeout: 2000 })
    })

    test('页面布局应正确显示左右两栏', async ({ page }) => {
        // 验证左侧商品选择区存在
        await expect(page.locator('.sales-left')).toBeVisible()

        // 验证右侧购物车面板存在
        await expect(page.locator('.sales-right')).toBeVisible()
    })
})
